...
export async function deleteTournament(tournamentId: string) {
  ...
  // 2. Delete RTDB bracket
  const bracketRef = ref(rtdb, `tournaments/${tournamentId}`);
  await set(bracketRef, null);
}
...
export function listenToBracket(
  tournamentId: string,
  callback: (bracket: TournamentBracket | null) => void,
) {
...
export async function joinTournamentMatch(
  ...
  if (!roomId) {
    ...
    const matchRef = ref(
      rtdb,
      `tournaments/${tournamentId}/bracket/${stage}/${matchId}`,
    );
...
export async function generateBracketFromParticipants(
  tournamentId: string,
  participants: TournamentParticipant[],
) {
  ...
  for (let i = 1; i <= 4; i++) {
    if (qfMatches[`qf${i}`].player1)
      updates[
        `tournaments/${tournamentId}/bracket/quarterFinals/qf${i}/player1`
      ] = qfMatches[`qf${i}`].player1;
    if (qfMatches[`qf${i}`].player2)
      updates[
        `tournaments/${tournamentId}/bracket/quarterFinals/qf${i}/player2`
      ] = qfMatches[`qf${i}`].player2;
  }
...
export async function updateMatchWinner(
  ...
  const bracketSnap = await get(
    ref(rtdb, `tournaments/${tournamentId}/bracket`),
  );
  const bracket = bracketSnap.val() as TournamentBracket;
  const match = bracket[stage][matchId];

  const winner =
    match.player1?.uid === winnerUid ? match.player1 : match.player2;
  if (!winner) return;
  ...
  if (stage === "quarterFinals") {
    ...
    updates[
      `tournaments/${tournamentId}/bracket/semiFinals/${nextMatchId}/${slot}`
    ] = winner;
...
export async function mockCreateUpcomingTournament() {
  ...
  await createTournament({
    ...
    description:
      "Compete against the top 8 typists of the week for an exclusive badge and global glory. Single elimination format.",
    status: "registration",
